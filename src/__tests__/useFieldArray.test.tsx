...
const mockGenerateId = () => {
    let id = 0;
    jest.spyOn(generateId, 'default').mockImplementation(() =>
        (id++).toString(),
    );
};
...
describe('useFieldArray', () => {
    ...
    describe('with resolver', () => {
        ...
        it('should provide correct form data with nested field array', async () => {
            ...
            fireEvent.click(
                screen.getByRole('button', { name: 'Append Nest' }),
            );
    ...
    describe('with setValue', () => {
        ...
        it('should set nested field array correctly', async () => {
                    ...
                    <ul>
                        {fields.map((item, i) => (
                            <Controller
                                ...
                                name={
                                    `test.${index}.keyValue.${i}.name` as const
                                }
                                control={control}
                            />
                        ))}
                ...
                return (
                    <form>
                        ...
                        <button
                            type="button"
                            onClick={() =>
                                setValue('test.0.keyValue', [{ name: '2a' }])
                            }
                        >
                            setValue
                        </button>
            ...
            expect(
                (screen.getByLabelText('test.0.firstName') as HTMLInputElement)
                    .value,
            ).toEqual('Bill');
        });
    });
    ...
    describe('array of array fields', () => {
            ...
            const ArrayField = ({
                ...
                return (
                    <div>
                        <input
                            {...register(
                                `fieldArray.${arrayIndex}.value` as const,
                            )}
                        />
                ...
                return (
                    <>
                        ...
                        <button
                            type={'button'}
                            onClick={() =>
                                prepend({ value: 'test', nestedArray: [] })
                            }
                        >
                            prepend
                        </button>
        ...
        it('should populate all array fields with setValue when name match Field Array', () => {
            ...
            const nestInput1 = screen.getByRole('textbox', {
                name: 'nest.1.value',
            });
        ...
        it('should worked with deep nested field array without chaining useFieldArray', () => {
            ...
            expect(screen.getAllByRole('textbox')).toHaveLength(2);
            const nestInput = screen.getByRole('textbox', {
                name: 'nest.0.value',
            });
            ...
            expect(
                screen.getByRole('textbox', { name: 'nest.0.value' }),
            ).toHaveValue('newV1');
            expect(
                        ...
                        {fields.map((field, index) => {
                            return (
                                <div key={field.id}>
                                    <input
                                        {...register(
                                            `test.${index}.yourDetail.firstName`,
                                        )}
                                    />
                                    <input
                                        {...register(
                                            `test.${index}.yourDetail.lastName`,
                                        )}
                                    />
                                </div>
                            );
                ...
                <div>
                    {fields.map((field, index) => {
                        return (
                            <div key={field.id}>
                                <input
                                    {...register(`root.${index}.test` as const)}
                                />
                                <Child
                                    control={control}
                                    register={register}
