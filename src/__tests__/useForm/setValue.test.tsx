...
describe('setValue', () => {
    ...
    describe('with validation', () => {
        ...
        it('should be called trigger method if shouldValidate variable is true and field value is array', async () => {
            ...
            expect(result.current.formState.errors?.test?.[0]?.message).toBe(
                'min',
            );
            expect(result.current.formState.errors?.test?.[1]?.message).toBe(
                'min',
            );
            expect(result.current.formState.errors?.test?.[2]?.message).toBe(
                'min',
            );
        });
    ...
    describe('with dirty', () => {
        ...
        it.each(['isDirty', 'dirtyFields'])(
            ...
            (property) => {
                ...
                expect(result.current.formState.isDirty).toBeFalsy();
                expect(
                    result.current.formState.dirtyFields.test,
                ).toBeUndefined();
            },
        );
    });
    ...
    it('should validate the input and return correct isValid formState', async () => {
        ...
        await act(async () => {
            result.current.setValue('test.data', 'test', {
                shouldValidate: true,
            });
        });
                ...
                <>
                    {fields.map((item, index) => (
                        <Controller
                            ...
                            render={({ field }) => (
                                <input data-testid={inputId} {...field} />
                            )}
                        />
                    ))}
            ...
            return (
                <div>
                    ...
                    <button
                        onClick={() => {
                            setValue(
                                'test.0.nestedArray' as `test.0.nestedArray`,
                                [{ name: 'append' }],
                            );
                        }}
                    >
