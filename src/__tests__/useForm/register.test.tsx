...
describe('register', () => {
    ...
    describe('when defaultValue is provided', () => {
        ...
        it('should only check checkboxes when array defaultValue includes input value', async () => {
            ...
            expect(
                (screen.getByLabelText('checkbox-A') as HTMLInputElement)
                    .checked,
            ).toBe(false);
        });
    });
        ...
        const Component = () => {
            ...
            return (
                <>
                    {show && (
                        <input
                            {...register('test', { shouldUnregister: true })}
                        />
                    )}
    ...
    describe('register disabled', () => {
        ...
        it('should work correctly with toggle disabled attribute and validation', async () => {
            ...
            fireEvent.click(
                screen.getByRole('button', { name: 'setDisabled' }),
            );
                ...
                <div>
                    {value.map((_, index) => (
                        <input
                            key={index}
                            onChange={(e) => {
                                const updatedValue = checkboxValue.map(
                                    (item, i) => {
                                        if (index === i) {
    ...
    it('should not programmatically set input file value with FileList', async () => {
        ...
        await waitFor(() =>
            expect(
                screen.queryByPlaceholderText('test'),
