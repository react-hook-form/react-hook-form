...
>(
                        ...
                        fieldReference.refs.length > 1
                            ? fieldReference.refs.forEach(
                                  (checkboxRef) =>
                                      ...
                                      (checkboxRef.checked = Array.isArray(
                                          fieldValue,
                                      )
                                          ? !!(fieldValue as []).find(
                                                (data: string) =>
                                                    data === checkboxRef.value,
                                            )
    ...
    const trigger: UseFormTrigger<TFieldValues> = async (
        ...
        if (_options.resolver) {
            ...
            validationResult = isValid = await executeBuildInValidation(
                _fields,
            );
        }
