...
>(
    ...
    if (
      ...
      (names || Array.from(_names.mount)).forEach((name) => {
        if (name) {
          isValidating
            ? set({
                object: _formState.validatingFields,
                name,
                value: isValidating,
              })
  ...
  const _setFieldArray: BatchFieldArrayUpdate = (
    ...
    if (args && method && !_options.disabled) {
      ...
      if (
        ...
        shouldSetValues &&
          set({
            object: _formState.touchedFields,
            name,
            value: touchedFields,
          });
      }
  ...
  const shouldRenderByError = (
    ...
    if (_options.delayError && error) {
      ...
      error
        ? set({ object: _formState.errors, name, value: error })
      ...
      if (fieldReference) {
        !fieldReference.disabled &&
          set({
            object: _formValues,
            name,
            value: getFieldValueAs(value, fieldReference),
          });
        ...
        for (const fieldName of Array.from(fieldsToCheck)) {
          get(_formState.dirtyFields, fieldName)
            ? set({
                object: values,
                name: fieldName,
                value: get(_formValues, fieldName),
              })
            : setValue(
